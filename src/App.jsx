...
const DashboardLayout = ({ children }) => {
  const { user, logout, settings, setSettings, dbStatus, liveAuction, auctionSettings } = useAuth();
  ...
  if (user?.role === 'admin') {
    ...
    if (auctionSettings?.appMode === 'tournament') {
       navItems.push({ path: '/tournament', label: 'Tournament Fixtures', icon: 'fixtures' });
    }
    navItems.push({ path: '/auction', label: 'Live Auction', icon: 'auction' });
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-slate-50 dark:bg-[#0a0a0c]">
         ...
         <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-[#0a0a0c]/95 backdrop-blur-2xl border-t border-slate-200/50 dark:border-slate-800/50 z-[100] flex items-center justify-around px-2 pb-safe pt-3 shadow-sm border-t border-slate-200 dark:border-slate-800 rounded-t-3xl">
           {navItems.map((item) => {
             ...
             else if (item.label.includes('Fixtures')) shortLabel = 'FIXTURES';
...
function TournamentRoute() {
  ...
  if (user && user.role !== 'spectator') {
    return (
      <Suspense fallback={<PageLoader />}>
        <DashboardLayout>
          <div className="max-w-6xl mx-auto space-y-4 md:space-y-6 pb-40 md:pb-0">
             <div className="flex items-center justify-between bg-white dark:bg-[#111] p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
               <div className="flex items-center gap-2 font-bold text-sm uppercase tracking-widest px-2 text-indigo-600 dark:text-indigo-400">
