...
export default function AdminSettings() {
  ...
  const [localSettings, setLocalSettings] = useState(auctionSettings || {
    incrementRules: [], defaultManagerBudget: 10000, maxSquadSize: 15, allowCustomBids: true, defaultBasePrice: 100, auctionTimerDuration: 30, defaultIncrement: 10, auctioneerPassword: '123', globalNotice: '', auctionStartDate: '', appMode: 'auction'
  });
        ...
        <div className="col-span-12 md:col-span-9 space-y-12">
          ...
          <section id="general" className="scroll-mt-40">
            ...
            <div className="card-minimal p-6 md:p-8 grid md:grid-cols-2 gap-8 shadow-sm">
              ...
              <div className="space-y-3">
                <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest">App Mode</label>
                <div className="flex gap-2">
                  <button 
                    onClick={() => setLocalSettings({...localSettings, appMode: 'auction'})} 
                    className={`flex-1 py-3 text-sm font-bold rounded-lg border-2 transition-all ${localSettings.appMode !== 'tournament' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400' : 'border-slate-200 dark:border-slate-800 text-slate-500 hover:border-slate-300 dark:hover:border-slate-700'}`}
                  >
                    Auction Mode
                  </button>
                  <button 
                    onClick={() => setLocalSettings({...localSettings, appMode: 'tournament'})} 
                    className={`flex-1 py-3 text-sm font-bold rounded-lg border-2 transition-all ${localSettings.appMode === 'tournament' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'border-slate-200 dark:border-slate-800 text-slate-500 hover:border-slate-300 dark:hover:border-slate-700'}`}
                  >
                    Tournament Mode
                  </button>
                </div>
                <p className="text-xs text-slate-400 font-medium">Determines what shows on the spectator screen.</p>
              </div>
          ...
          <div className="pt-8 border-t border-slate-200 dark:border-slate-800 flex justify-end">
             <button onClick={handleSave} className="btn-primary py-4 px-10 text-sm tracking-widest uppercase font-black shadow-xl shadow-indigo-600/20 hover:scale-105 transition-transform flex items-center justify-center gap-3">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg>
                Save All Changes
             </button>
          </div>
